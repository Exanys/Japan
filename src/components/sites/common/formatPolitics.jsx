...
function FormatPolitics({data,selectedText, name, selectedMain, selectionMain, selectionText}) {
    ...
    return (
        <div className="container-fluid text-center">
            <div className="col-12 p-0"><h1 onClick={() =>{selectionMain(name); window.scrollTo(0,450)}}className={"text-white text-center rounded pb-3 pt-2 bg-" + (selectedMain ? 'primary ' : "secondary ")+(device > 768 && ' display-2')}>{name}</h1></div>
